...
class Recommender(object):
    ...
    '''
    def __init__(self, X, n_components=None, normalize=False, normalize_std=False):
        '''
        ...
        self.X = X # dataset
        k = min(min(X.shape), 10 if n_components is None else n_components)
        svdtrans = TruncatedSVD(n_components=k, n_iter=7, random_state=42)
        self.X_approx = svdtrans.fit_transform(self.X)
        XT_approx = svdtrans.fit_transform(self.X.T)
        self.users_similarity_matrix = cosine_similarity(self.X_approx)
        print(self.users_similarity_matrix.shape)
        self.items_similarity_matrix = cosine_similarity(XT_approx)
        print(self.items_similarity_matrix.shape)
        ...
        self.row_norms = np.sqrt(np.squeeze(np.asarray((self.X_approx**2).sum(1))))
    ...
    def recommend(self, row_i, top_users=3, top_items=3, alpha=1, limits=(0,5)):
        ...
        Y = self.X_approx @ self.X_approx[row_i].T
        # print(self.X[row_i].T)
        # print('x')
        ...
        if limits is None:
            max = R[top_item_indices[0]]
            if max == 0: out['r'] = [(i, 0) for i in top_item_indices]
            else: out['r'] = [(i, R[i]/max) for i in top_item_indices]            
        else:
